...
let
  ...
  lock = pkgs.writeShellScriptBin "lock.sh" ''
    ...
    ${pkgs.i3lock}/bin/i3lock -i /tmp/lock_screenshot.png -e -f
...
in rec {
  ...
  xsession.windowManager.i3.config = {
    ...
    keybindings = {
      ...
      "Print" =
        ''exec maim ~/Pictures/screenshots/$(date +"%Y-%m-%d_%H:%M:%S").png'';
      "Control+Print" = "exec maim | xclip -selection clipboard -t image/png";
      "${modifier}+Print" = ''
        exec maim -s ~/Pictures/screenshots/$(date +"%Y-%m-%d_%H:%M:%S").png'';
      "${modifier}+Control+Print" =
        "exec maim -s | xclip -selection clipboard -t image/png";

      "XF86KbdBrightnessDown" =
        "exec echo $(expr $(cat /sys/class/leds/asus::kbd_backlight/brightness) - 1) > /sys/class/leds/asus::kbd_backlight/brightness";
      "XF86KbdBrightnessUp" =
        "exec echo $(expr $(cat /sys/class/leds/asus::kbd_backlight/brightness) + 1) > /sys/class/leds/asus::kbd_backlight/brightness";

      "${modifier}+F3" =
        "exec echo $(expr $(cat /sys/devices/platform/asus-nb-wmi/hwmon/hwmon3/pwm1) - 3) > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon3/pwm1";
      "${modifier}+F4" =
        "exec echo $(expr $(cat /sys/devices/platform/asus-nb-wmi/hwmon/hwmon3/pwm1) + 3) > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon3/pwm1";
      "${modifier}+F2" =
        "exec echo $([[ $(cat /sys/devices/platform/asus-nb-wmi/hwmon/hwmon3/pwm1_enable) = 2 ]] && echo 1 || echo 2) > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon3/pwm1_enable";
      ...
      "XF86MonBrightnessDown" =
        "exec echo $(expr $(cat /sys/class/backlight/intel_backlight/brightness) - 10) > /sys/class/backlight/intel_backlight/brightness";
      "XF86MonBrightnessUp" =
        "exec echo $(expr $(cat /sys/class/backlight/intel_backlight/brightness) + 10) > /sys/class/backlight/intel_backlight/brightness";
    ...
    window = {
      commands = [
        ...
        {
          command = "<span>[%instance] %title</span>";
          criteria = { class = "^.*"; };
        }
        {
          command = "border none";
          criteria = { class = "(Kitty|Alacritty|GLava)"; };
        }
      ];
    };
